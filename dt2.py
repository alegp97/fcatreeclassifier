...
class FCADecisionTree:
    def __init__(self, attribute_names=None, concept_names=None):
        self.root = None
    ...
    def _induce_tree(self, data, target, attributes):
        if len(set(target)) == 1:
        ...
        for category in set(row[best_attr] for row in data):
            child_data = [data[i] for i in range(len(data)) if data[i][best_attr] == category]
            child_target = [target[i] for i in range(len(data)) if data[i][best_attr] == category]
            child_nodes[category] = self._induce_tree(child_data, child_target, attributes - {best_attr})
    ...
    def train_tree(self, data, target):
        self.train_default(data, target)

        

    def train_default(self, data, target):
        self.context = [[1 if cell == "True" else 0 for cell in row] for row in data]
        attributes = set(range(len(data[0])))
        self.root = self._induce_tree(data, target, attributes)

        # Establecer la decisión predeterminada como la clase más común en el conjunto de entrenamiento
