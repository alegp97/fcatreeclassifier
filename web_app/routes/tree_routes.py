...
from fcapy.context import read_csv
...
@app.route('/build-lattice', methods=['POST'])
def build_lattice():
    file_name = request.form['file']
    file_path = os.path.join(USER_DATASETS_DIRECTORY_PATH, file_name)


    adapt_csv_for_fcapy(file_path)

    # Leer el archivo CSV y crear un contexto formal
    k = read_csv(file_path)

    # Crear retículo de conceptos
    lattice = ConceptLattice.from_context(k)

    print("jaja")

    # Visualizar el retículo de conceptos
    # Puedes ajustar la visualización según tus necesidades. Esta es una manera básica.
    viz = ConceptLatticeVisualizer(lattice)
    viz.draw_networkx()  # Este método dibuja el retículo usando la librería networkx. Puedes usar otro método si prefieres.

    return render_template('fca_tree_builder.html', lattice=lattice) """
